...
if [ -z $REDIS_HOME ] || [ -z "$SLAVE_NAME" ] || [ -z "$MASTER_IP" ] || [ -z "$SLAVE_QUORUM" ] || [ -z "$REDIS_PORT" ] || [ -z "$REDIS_PASSWORD" ]; then
  ...
  echo MASTER_NAME的值是: "$SLAVE_NAME"
...
sentinel monitor $SLAVE_NAME $MASTER_HOST $SLAVE_QUORUM

# 如果master节点的Redis 实例受到保护，则 Sentinel 将使用此密码对其进行身份验证。如果主实例没有设置密码，则不需要此选项。
...
sentinel down-after-milliseconds $SLAVE_NAME 3000

# 同一个sentinel对同一个master两次failover之间的间隔时间（180秒）
sentinel failover-timeout $SLAVE_NAME 180000
...
sentinel parallel-syncs $SLAVE_NAME 5
EOF
